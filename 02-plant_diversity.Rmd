...
# download.file('https://try-db.org/dnld/TryAccSpecies.tsv', destfile = "data/TRY/TryAccSpecies.tsv", method = "wget", extra = "-r -p --random-wait", quiet = T)
...
try_sp <- species_list %>%
  ...
  na.omit() %>%
  unique() %>%
  left_join(readr::read_tsv("data/TRY/TryAccSpecies.tsv"),
...
try_release%>%
  ...
  readr::write_tsv("../../Data/list_traits_TRY/traits_with_some_values.tsv")
```
